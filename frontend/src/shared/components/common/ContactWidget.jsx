...
const ContactWidget = () => {
  ...
  useEffect(() => {
    if (showAiChat && user && !amIAdmin) {
      ...
      newSocket.on('connect', () => {
        newSocket.emit('join_ai_chat', {
          userId: user.id,
          userName: user.name || user.email
        });
      });

      newSocket.on('ai_chat_history', (history) => {
        setAiMessages(history);
      });

      newSocket.on('receive_ai_message', (msg) => {
        if (msg.sender === 'ai') {
          setAiLoading(false);
        }
  ...
  }, [showAiChat, user, amIAdmin]);

  // Auto scroll
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [messages, adminChats, aiMessages, aiLoading]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {showAiChat && (
        <div className="bg-white w-80 sm:w-96 rounded-2xl shadow-2xl overflow-hidden flex flex-col mb-4 border border-gray-100" style={{ height: '520px' }}>
          {/* Header */}
          <div className="bg-emerald-600 text-white px-4 py-3 flex justify-between items-center shadow-md z-10">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-semibold text-sm">Trợ lý ảo Zero Phone</h3>
                <p className="text-xs text-emerald-100">Trả lời tự động bằng Gemini AI</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <button 
                onClick={handleHandoff}
                className="text-xs bg-white/20 hover:bg-white/30 text-white px-2.5 py-1.5 rounded-full font-medium transition cursor-pointer"
                title="Gặp nhân viên tư vấn"
              >
                Gặp nhân viên
              </button>
              <button onClick={() => setShowAiChat(false)} className="text-white/80 hover:text-white hover:bg-white/20 p-1 rounded-full transition cursor-pointer">
                <XMarkIcon className="w-5 h-5" />
              </button>
            </div>
          </div>
            ...
            {!user ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mb-4">
                  <span className="text-emerald-500 font-bold text-2xl">AI</span>
                </div>
                <h4 className="font-semibold text-gray-800 mb-1">Vui lòng đăng nhập</h4>
                <p className="text-sm text-gray-500 mb-6">Bạn cần đăng nhập bằng tài khoản để bắt đầu trò chuyện cùng Trợ lý AI.</p>
                <button 
                  onClick={() => {
                    setShowAiChat(false);
                    window.dispatchEvent(new Event('openLoginModal'));
                  }}
                  className="w-full bg-emerald-600 text-white py-2.5 rounded-xl font-medium hover:bg-emerald-700 shadow-md transition cursor-pointer"
                >
                  Đăng nhập ngay
                </button>
              </div>
              ...
              <>
                <div className="text-center text-xs text-gray-400 my-2">Cuộc trò chuyện với Trợ lý AI thông minh</div>
                {aiMessages.map((msg, idx) => {
                  const isMine = msg.sender === 'user';
                  return (
                    <div key={idx} className="flex flex-col gap-2">
                      <div className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl ${isMine ? 'bg-emerald-600 text-white rounded-br-sm' : 'bg-white border border-gray-200 text-gray-800 rounded-bl-sm shadow-sm'}`}>
                          <p className="text-sm whitespace-pre-wrap">{msg.text}</p>
                          <span className={`text-[10px] mt-1 block ${isMine ? 'text-emerald-200 text-right' : 'text-gray-400 text-left'}`}>
                            {new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                          </span>
                        </div>
                      </div>

                      {/* Display custom interactive cards if payload is present */}
                      {!isMine && msg.payload && (
                        <div className="w-full flex justify-start pl-2 animate-fade-in-up">
                          <div className="w-full max-w-[85%] bg-white border border-gray-200 rounded-xl p-3 shadow-md flex flex-col gap-2.5">
                            
                            {/* Products Payload */}
                            {msg.payload.type === 'products' && (
                              <>
                                <div className="text-xs font-semibold text-gray-500 border-b pb-1">Sản phẩm gợi ý:</div>
                                <div className="flex flex-col gap-2 overflow-y-auto max-h-48 pr-1">
                                  {msg.payload.data.map((prod) => (
                                    <div key={prod.id} className="flex items-center gap-2.5 p-1.5 hover:bg-gray-50 rounded-lg transition border border-gray-50">
                                      <img src={prod.image} alt={prod.name} className="w-10 h-10 object-cover rounded-md bg-gray-100 flex-shrink-0" />
                                      <div className="flex-1 min-w-0">
                                        <h5 className="text-xs font-bold text-gray-800 truncate">{prod.name}</h5>
                                        <p className="text-xs text-rose-600 font-semibold">{Number(prod.price).toLocaleString()}đ</p>
                                        <p className="text-[10px] text-gray-400">Kho: {prod.stock} máy</p>
                                      </div>
                                      <button 
                                        onClick={() => {
                                          setShowAiChat(false);
                                        ...
                                        className="text-[10px] bg-emerald-600 hover:bg-emerald-700 text-white px-2 py-1 rounded font-medium cursor-pointer flex-shrink-0"
                                      >
                                        Xem
                                      </button>
                                    </div>
                            ...
                            {/* Discounts Payload */}
                            {msg.payload.type === 'discounts' && (
                              <>
                                <div className="text-xs font-semibold text-gray-500 border-b pb-1">Mã giảm giá đang hoạt động:</div>
                                <div className="flex flex-col gap-2">
                                  {msg.payload.data.map((voucher) => (
                                    <div key={voucher.id} className="border border-dashed border-emerald-400 bg-emerald-50/50 p-2 rounded-lg flex items-center justify-between">
                                      <div>
                                        <span className="bg-emerald-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">{voucher.code}</span>
                                        <h5 className="text-xs font-bold text-gray-800 mt-1">{voucher.description || `Giảm ${Number(voucher.value).toLocaleString()}đ`}</h5>
                                        <p className="text-[9px] text-gray-400">Đơn tối thiểu: {Number(voucher.minOrderValue).toLocaleString()}đ</p>
                                      </div>
                                      <button 
                                        onClick={() => {
                                        ...
                                        className="text-[9px] bg-emerald-100 text-emerald-800 border border-emerald-300 hover:bg-emerald-200 px-2 py-1 rounded font-semibold cursor-pointer"
                                      >
                                        Sao chép
                                      </button>
                                    </div>
                            ...
                            {/* Orders Payload */}
                            {msg.payload.type === 'orders' && (
                              <>
                                <div className="text-xs font-semibold text-gray-500 border-b pb-1">Đơn hàng của bạn:</div>
                                <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
                                  {msg.payload.data.slice(0, 3).map((ord) => (
                                    <div key={ord.id} className="p-2 border rounded-lg bg-gray-50 flex flex-col gap-1 text-[11px]">
                                      <div className="flex justify-between font-bold text-gray-700">
                                        <span>#{ord.orderCode.slice(-8)}</span>
                                        <span className={`px-1.5 py-0.5 rounded text-[9px] uppercase ${
                                          ord.orderStatus === 'pending' ? 'bg-amber-100 text-amber-800' :
                                          ord.orderStatus === 'confirmed' ? 'bg-blue-100 text-blue-800' :
                                      ...
                                      <div className="text-gray-500">Tổng tiền: <span className="font-semibold text-gray-700">{Number(ord.total).toLocaleString()}đ</span></div>
                                      <div className="text-gray-500">Ngày đặt: {new Date(ord.createdAt).toLocaleDateString()}</div>
                                      <button
                                        onClick={() => {
                                          setShowAiChat(false);
                                          navigate('/my-orders');
                                        }}
                                        className="text-[9px] bg-white border border-gray-300 hover:bg-gray-100 py-0.5 text-center rounded font-medium mt-1 cursor-pointer"
                                      >
                                        Chi tiết đơn hàng
                                      </button>
                                    </div>
                ...
                {aiLoading && (
                  <div className="flex justify-start pl-1">
                    <div className="bg-white border border-gray-200 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-1">
          ...
          {/* Footer Input */}
          {user && (
            <form onSubmit={handleSendAi} className="bg-white border-t border-gray-100 p-3 flex gap-2">
              <input 
                type="text" 
                value={text}
                onChange={(e) => setText(e.target.value)}
                placeholder="Hỏi trợ lý AI..." 
                className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:bg-white transition"
              />
              <button 
                type="submit" 
                disabled={!text.trim() || aiLoading}
                className="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center hover:bg-emerald-700 disabled:opacity-50 disabled:hover:bg-emerald-600 transition shadow-md cursor-pointer"
              >
                <PaperAirplaneIcon className="w-5 h-5 -ml-0.5" />
              </button>
            </form>
      ...
      {showOptions && !showChat && !showAiChat && (
        <div className="bg-white rounded-xl shadow-xl border border-gray-100 mb-4 overflow-hidden w-64 animate-fade-in-up">
          ...
          <div className="flex flex-col">
            ...
            <button 
              ...
              className="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition border-b border-gray-50 text-left w-full cursor-pointer"
            >
              ...
              <div>
                <p className="text-sm font-medium text-gray-800">Trợ lý AI (Gemini)</p>
                <p className="text-xs text-gray-500">Tư vấn, tra cứu đơn 24/7</p>
              </div>
            </button>
